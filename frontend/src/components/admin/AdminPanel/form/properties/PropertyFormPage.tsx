// src/components/admin/AdminPanel/form/PropertyFormPage.tsx
"use client";
...
// ✅ import'u düzelt (senin verdiğin yol)
...
import { PropertyMetaSection } from "./PropertyMetaSection";
...
import { PropertyPublishSection } from "./PropertyPublishSection";

type UploadedLike = {
  asset_id: string | null;
  url: string | null;
  fileName?: string; // ✅ property varsa string, yoksa tamamen absent olmalı
  mime?: string | null;
};
...
export default function PropertyFormPage() {
  ...
  const uploadCover = async (file: File): Promise<void> => {
    ...
    try {
      ...
      form.upsertCoverFromUpload(assetId, url, fallbackAlt);

      await Promise.resolve();

      const nextAssets = (form.buildPatchBody() as any).assets;
  ...
  const uploadGalleryFiles = async (files: FileList): Promise<void> => {
    ...
    try {
      ...
      const rawItems =
        anyRes?.items ?? anyRes?.data?.items ?? anyRes?.result?.items ?? [];

      const normalized = (Array.isArray(rawItems) ? rawItems : [rawItems])
        .flat()
        .filter(Boolean);

      // ✅ exactOptionalPropertyTypes uyumu:
      // fileName undefined ise property'yi hiç eklemiyoruz (omit)
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <PropertyBasicSection
        ...
        autoSlug={form.autoSlug} setAutoSlug={form.setAutoSlug}
      ...
      <PropertyPriceSection
        ...
        currency={form.currency} setCurrency={form.setCurrency}
        minPriceAdmin={form.minPriceAdmin} setMinPriceAdmin={form.setMinPriceAdmin}
      />

      <PropertyMetaSection
        listingNo={form.listingNo} setListingNo={form.setListingNo}
        badgeText={form.badgeText} setBadgeText={form.setBadgeText}
        featured={form.featured} setFeatured={form.setFeatured}
      />

      <PropertyDetailsSection
        grossM2={form.grossM2} setGrossM2={form.setGrossM2}
        netM2={form.netM2} setNetM2={form.setNetM2}
        rooms={form.rooms} setRooms={form.setRooms}
        buildingAge={form.buildingAge} setBuildingAge={form.setBuildingAge}
        floor={form.floor} setFloor={form.setFloor}
        totalFloors={form.totalFloors} setTotalFloors={form.setTotalFloors}
        heating={form.heating} setHeating={form.setHeating}
        furnished={form.furnished} setFurnished={form.setFurnished}
        inSite={form.inSite} setInSite={form.setInSite}
        hasBalcony={form.hasBalcony} setHasBalcony={form.setHasBalcony}
        hasParking={form.hasParking} setHasParking={form.setHasParking}
        hasElevator={form.hasElevator} setHasElevator={form.setHasElevator}
      />
      ...
      <PropertyPublishSection
        ...
        displayOrder={form.displayOrder} setDisplayOrder={form.setDisplayOrder}
      />
    </div>
  );
}
